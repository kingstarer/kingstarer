...
/*
` https://www.cnblogs.com/rxbook/p/7542276.html
//http代理 返回状态码 请求结果 错误信息
func HttpGetFund(getPath string) (int, string, error) {
	var data string
	var statusCode int = http.StatusInternalServerError
...
func HttpGetFund(fundCode string) (string, error)  {

	fundGzUrl := fmt.Sprintf("http://fundgz.1234567.com.cn/js/%s.js?rt=1637642869962", fundCode)
	resp, err := http.Get(fundGzUrl)
	if err != nil {
		// handle error
	}
